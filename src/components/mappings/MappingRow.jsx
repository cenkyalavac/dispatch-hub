...
export default function MappingRow({ mapping, onToggle, onDelete, onSave }) {
  ...
  const startEdit = () => {
    setSourceValue(mapping.source_value || '');
    setDestinationValue(mapping.destination_value || '');
  ...
  if (editing) {
    return (
      <div className="bg-surface-1 border border-accent rounded-md px-4 py-3 flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[11px] font-medium text-ink-2 min-w-[100px]">
        ...
        <div className="ml-auto flex items-center gap-1">
          <button
            onClick={submit}
            disabled={saving || !sourceValue.trim() || !destinationValue.trim()}
