...
import { ChevronDown, ChevronUp, Check, AlertCircle, AlertTriangle, Mail } from 'lucide-react';
...
export default function SystemIssueRow({ issue, busy, onResolve }) {
    ...
    <>
      <tr className="border-b border-line-1 last:border-0 hover:bg-surface-2 transition-colors">
        ...
        <td className="px-3 py-2.5">
          <p className="text-[13px] font-medium text-ink-1 truncate max-w-[360px]" title={issue.title}>
            {issue.title}
          </p>
          <p className="text-[11px] text-ink-3 mt-0.5 flex items-center gap-2">
            ...
            {issue.emailed_at && (
              <span className="inline-flex items-center gap-0.5" title={`Emailed ${format(new Date(issue.emailed_at), 'dd MMM HH:mm')}`}>
