// BMS reports the project as delivered. Transitions: synchronized -> delivered.
...
Deno.serve(async (req) => {
  try {
    ...
    if (project.state !== 'synchronized') {
      return Response.json({
        error: `Project is in '${project.state}' state — only 'synchronized' projects can be delivered.`,
      }, { status: 409 });
    }
    ...
    return Response.json({ success: true, project: { id: updated.id, state: updated.state, delivered_at: updated.delivered_at } });
  } catch (error) {
    console.error('apiProjectsDeliver error:', error.message);
